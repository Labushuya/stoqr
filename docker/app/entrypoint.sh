...
# WICHTIG: das gemountete Verzeichnis muss dem Container-User (uid 1001) gehoeren,
# sonst 404 auf /media. Host: chown -R 1001:1001 <media-dir>.
...
if mkdir -p "${MEDIA_DIR}" 2>/dev/null && [ -w "${MEDIA_DIR}" ]; then
  ...
  echo "[stoqr] WARN: ${MEDIA_DIR} nicht beschreibbar — Bild-Download deaktiviert. Host: chown -R 1001:1001 auf den Mount pruefen."
fi
